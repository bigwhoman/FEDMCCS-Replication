...
docker run -d --name 'client2' --env PORT=8089 --env CORES=2 --env FREQUENCY=3750 --env MEMORY=500 --env PING=0 --env BANDWIDTH=0 --add-host=host.docker.internal:host-gateway --cpuset-cpus '1,2' --cpus '1.5' --memory '500M' fed-client
if ! [[ "$(sudo ufw status)" =~ "inactive" ]]; then
...
docker run -d --name 'client3' --env PORT=8090 --env CORES=1 --env FREQUENCY=1250 --env MEMORY=50 --env PING=50 --env BANDWIDTH=524288 --add-host=host.docker.internal:host-gateway --cpuset-cpus '3' --cpus '0.25' --memory '50M' fed-client
